...
- The social cards are `app/[locale]/opengraph-image.tsx` (home), `app/[locale]/blog/opengraph-image.tsx` (list) and `app/[locale]/blog/[slug]/opengraph-image.tsx` (one per post, same gradient, accent and photo); each exports `generateStaticParams` for the full locale (and slug) set because image routes do not inherit the layout's params, and renders the dictionary strings of its locale. Next injects `og:image` automatically (pointing at the `/en/...` or `/fr/...` image), so `metadata.openGraph.images` stays unset. The static `alt` export reads from the English dictionary.
...
- `public/sitemap.xml`, `public/sitemap-0.xml`, `public/robots.txt` are **generated** by `next-sitemap` on `postbuild` from `next-sitemap.config.js`, whose `transform` strips the `/en` prefix, skips the fallback post routes and writes `alternateRefs` (`en`, `fr`, `x-default`) from the files in `content/blog/`. Never edit them; the `protect-generated` hook blocks it. `public/site.webmanifest` and the favicons are static.
