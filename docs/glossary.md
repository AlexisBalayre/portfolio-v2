...
| **Content date** | The only kind of `lastmod` the sitemap carries: a post's frontmatter `date`, the newest post date for the blog list, and for the home page the later of that and `portfolioUpdatedOn` (`lib/site.ts`, bumped by hand with the content JSON). Never the build time. |
