...
- **`app/sitemap.ts`** and **`app/robots.ts`** are Next metadata routes, prerendered as `/sitemap.xml` and `/robots.txt` ([ADR 0003](../adr/0003-sitemap-and-robots-as-next-metadata-routes.md)). The sitemap lists the home page, the blog list and every post in every locale that has a file for it, with `alternates.languages` (`en`, `fr`, `x-default`) from `languageAlternates` and a content-derived `lastmod`: the post's frontmatter date, the newest post date for the list, and for the home page the later of that date and `portfolioUpdatedOn` in `lib/site.ts`. The social images and the feeds are never listed. `robots.ts` allows every user agent, names the AI crawlers in one explicit group and points at the sitemap.
