...
allowed-tools: Bash(git *), Bash(gh *), Bash(yarn *), Read, Edit, Write, Grep, Glob, Agent, mcp__github_inline_comment__create_inline_comment
...
- **Leave linters alone:** do not raise anything ESLint, Prettier, or tsc already catches; that is noise, not a finding.
...
- **Pre-flight** (local source): run `yarn lint && yarn typecheck` (this repo has no test suite). If it fails, **stop**: report the failures and ask the user to return once the tree is green. Reviewers assume the deterministic layer is clean; spawning them on a red tree pays opus to rediscover what tooling already flagged. For a pr source running under CI, skip the pre-flight: the build workflow already gates the tree.
...
- **Shape**: the changed files, their line counts, and the areas they span (`app/`, `components/`, `public/assets/data/`, `docs/`, root config), from `git diff --stat` against the merge-base (local) or `gh pr diff <n>` plus `gh pr view <n> --json files` (pr).
...
| `review-security` | opus | a trust boundary is touched (raw HTML rendered from content JSON, external links or scripts, `next.config.js`, dependencies, any new API route) |
| `review-context` | sonnet | spec/infra surfaces touched (`next.config.js`, SEO metadata and JSON-LD, sitemap config, CI, settings) |
...
- **Post-verify**: after editing, re-run `yarn lint && yarn typecheck` to prove no regression was introduced. Report the result.
...
- [ ] Gate the run: PR freshness (pr) or pre-flight `yarn lint && yarn typecheck` (local).
