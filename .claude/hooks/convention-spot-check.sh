  ...
  case "$file" in
    ...
    *.tsx)
      if grep -q 'dangerouslySetInnerHTML' "$file" && [ "$file" != "components/Timeline.tsx" ] && [ "$file" != "app/layout.tsx" ]; then
        WARNINGS+="  $file: new dangerouslySetInnerHTML usage; keep raw HTML rendering confined to Timeline.tsx and the layout.tsx JSON-LD (content.md §Shapes)\n"
      fi
...
if [ "$DATA_CHANGED" = true ] && [ "$LLMS_CHANGED" = false ]; then
  WARNINGS+="  public/assets/data/*.json changed but public/llms.txt did not; keep the AI-agent summary in sync (content.md §Keep in sync)\n"
fi
