...
if echo "$COMMAND" | grep -qE 'rm[[:space:]]+-rf[[:space:]]'; then
  echo "BLOCKED: destructive command (rm -rf) not allowed." >&2
  exit 2
fi
...
if echo "$COMMAND" | grep -qE 'git[[:space:]]+checkout[[:space:]]+-b\b'; then
  ...
  if [ "$CURRENT_BRANCH" = "$TRUNK" ]; then
    echo "BLOCKED: do not use 'git checkout -b' on $TRUNK. Use 'yarn worktree:create <name>' instead." >&2
    exit 2
  fi
fi

# Anchor on the refspec, not the substring: branch names like feature/fix-main-red
# and tag pushes must pass; only an actual trunk destination is blocked.
